...
//
		...
		DijkstraSP shortestPaths = new DijkstraSP(finalGraph, keys.indexOf("OKC"));
		
		
		if (!shortestPaths.hasPathTo(keys.indexOf("MRI"))){
			System.out.println("Sorry! No path exists to here!");
		}
		else{
			for (AirportEdge e : shortestPaths.pathTo(keys.indexOf("MRI"))){
				System.out.println(keys.get(e.from()) + " - > " + keys.get(e.to()));
				System.out.println(locations.get(e.from()) + " - > " + locations.get(e.to()));
			}
		}
